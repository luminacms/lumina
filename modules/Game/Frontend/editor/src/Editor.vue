    ...
    export default {
        ...
        methods: {
            ...
            loadPageInfo: function() {
                ...
                Server({
                    ...
                    if (info.canvas && info.canvas.width) this.$store.dispatch('SettingChange', {
                        phoneSize: info.canvas
                    })
                    this.nodeInfo = cloneDeep(info)
                    this.$store.dispatch('setUser', {
                        userId: data.data && data.data.uid
                    })
                })
            },
            /**
            ...
            savePagePreviewImage() {
                var urlInfo = common.parseURL(window.location.href)
                this.ema.fire('screenshot', document.querySelector('#stage'), {
                    height: 486,
