...
class Tree {
     ...
     * @return array
     */
    public function get_parent_ids($myid) {
        $pids = [];
        $parent = $this->get_parent($myid);
        if(count($parent)>0) {
            array_push($pids, $parent['id']);
            $pids = array_merge($pids, $this->get_parent_ids($parent['id']));
        }
        return $pids;
    }
     ...
     * @return array
     */
    public function get_child_ids($myid = 0) {
        $retarray = [];
        //一级栏目数组
        ...
        if (is_array($child)) {
            foreach ($child as $k => $value) {
                $retarray[] = $value['id'];
                if($childArr = $this->get_child_ids($value['id'])) {
                    $retarray = array_merge($retarray, $childArr);
                }
            }
        }
        return $retarray;
    ...
    public function get_tree_array($parentid = 0) {
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $k => $value) {
                $retarray[$k] = $value;
                if($childArr = $this->get_tree_array($value['id'])) {
                    $retarray[$k]["children"] = array_values($childArr);
                }
            }
        }
        return $retarray;
