        ...
        if(count($item) > 0){
            ...
            foreach ($filtered as $_key => $_item) {
                if(isset($objCast[$_key]) && $objCast[$_key] == 'array') {
                    ...
                    $_html .= '<tr><td width="120">'.__("core::main.".$_key).'</td><td><pre class="layui-code"><dl>'.($_json).'</dl></pre></td></td></tr>';
                }else{
                    $_html .= '<tr><td width="120">'.__("core::main.".$_key).'</td><td>'.$_item.'</td></td></tr>';
                }
            }
        }else{
