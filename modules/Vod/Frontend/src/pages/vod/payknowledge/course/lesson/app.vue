<template>
    <div class="app">
        ...
        <div class v-if="!loading">
            <div class="video">
                <section id="xgvideo" class="chimee-container" v-show="item.media_src"></section>
                <div class="poster" v-show="!item.media_src"><img :src="item.thumb" /></div>
            </div>
                ...
                <van-tab title="精品课程">
                    <div class="tj">
                        ...
                        <van-list v-model="recomLoading" :finished="recomFinished" finished-text="没有更多了"
                            ...
                            <div class v-for="( item, index) in lessonItems" :key="index" @click="goto('/course/lesson', {'id': item.id, 'oid': item.oid})">
            ...
            <van-goods-action>
                ...
                <van-goods-action-button type="danger" :text="'立即购买 ￥' +item.price" @click="toBuy" />
            </van-goods-action>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            // 拉取详情
            store.setData('name', 'jory')

            lessonApi.show(_params.id).then((res) => {
                self.item = res.data
                self.loading = false

                if(self.item.media_src) {
                    // 已购买暴露视频链接
        ...
        methods: {
            initVideoPlayer() {
                ...
                self.$nextTick(() => {
                    let videoPlayer = new Player({
                        ...
                        autoplay: false,
                        playsinline: true,
                        url: [{name:'',src:self.item.media_src,'poster': self.item.thumb}]
                    });
                })
            },
            ...
            toBuy() {
                ...
                let waitPay = Toast.loading({
                    mask: true,
                    message: '加载中...',
                    duration: 0
                })
                orderApi.makeOrder('lesson', this.item.id).then((res) => {
                    console.log(res)
                })
            },
            toHome() {
            ...
            loadRecommend() {
                let self = this
                lessonApi.recommend(this.item.id, {'page': 1}).then((res) => {
