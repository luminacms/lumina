    ...
    <script>
        layui.use(['table', 'element'], function(){
            ...
            table.on('toolbar(data_course_table)', function(obj){
                ...
                if(obj.event == 'create') {
                    parent.layui.admin.openTabsPage('{{ route('vod.courses.create') }}', '新增数据')
                    return true;
                }
                ...
                if(_.indexOf(['delete', 'update'], obj.event) > -1 && checked.data.length >0 ) {
                    ...
                    if(obj.event === 'delete') {
                        ...
                        parent.layui.admin.openTabsPage('{{route('vod.courses.edit', '_id_')}}'.replace('_id_', checked.data[0].id), '修改数据#'+checked.data[0].id)
                        return true;
                    }
                }
            });
