...
if(!function_exists('is_can_create_project')) {
    ...
    function is_can_create_project($member_id){
        return \Modules\Doc\Models\Project::isCanCreateProject($member_id);
    }
}
    ...
    function system_install($dbHost,$dbName,$dbPort,$dbUser,$dbPassword, $account, $password, $email)
    {
        ...
        try {
            ...
            if ($pdo->beginTransaction()) {
                ...
                $sql = "INSERT INTO member(account,member_passwd,group_level,nickname,email,created_at,state,headimgurl) SELECT '{$account}','{$password}',0,'{$account}','{$email}',now(),0,'{$headimgurl}' FROM dual WHERE NOT exists(SELECT * FROM member WHERE `account` = '{$account}');";
