...
use Modules\Doc\Models\Passwords;
...
class AccountController extends BaseController
{
    ...
    public function findPassword()
    {
        if($this->isPost()){
            ...
            $totalCount = Passwords::where('created_at','>=', date('Y-m-d H:i:s',time() - (int)wiki_config('MAIL_TOKEN_TIME',3600)))->count();
            ...
            $passwords->created_at = date('Y-m-d H:i:s');
            if(!$passwords->save()){
            ...
            {
                $message->to($passwords->email)->subject('App - 找回密码!');
    ...
    public function modifyPassword($key)
    {
        ...
        $passwords = Passwords::where('token','=',$key)->where('is_valid','=','0')->where('created_at','>', date('Y-m-d H:i:s',time() - (int)wiki_config('MAIL_TOKEN_TIME',3600)))->first();
