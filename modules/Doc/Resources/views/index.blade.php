                            ...
                            <div>
                                <div>
                                    <div class="pull-left">
                                        ...
                                        @endif
                                        <a href="{{route('document.index',['id'=>$item->id])}}" title="编辑文档" data-toggle="tooltip" data-placement="bottom" >{{$item->project_name}}</a>
                                    </div>
                                    <div class="pull-right">
                                        <a href="{{route('doc.show',['id'=>$item->id])}}" title="查看文档" style="font-size: 12px;" data-toggle="tooltip" data-placement="bottom"  ><i class="fa fa-eye"></i> 查看</a>
                                        {{--                                        @if($member->group_level === 0)--}}
                                        <a href="{{route('project.members',['id'=>$item->id])}}" class="project-user-btn" title="管理文档成员" data-toggle="tooltip" data-placement="bottom"  style="font-size: 12px;"><i class="fa fa-user-plus"></i> 用户</a>
                                        {{--                                        @else--}}
                                        {{--                                            @if($item->role_type == 0 && $member->group_level != 0)--}}
                                        <a href="###" class="project-quit-btn" title="退出" data-url="{{route('project.quit',['id' => $item->id])}}" data-toggle="tooltip" data-placement="bottom"  style="font-size: 12px;"><i class="fa fa-power-off"></i> 退出</a>
                                        {{--                                            @elseif($item->role_type == 1 || $member->group_level == 0)--}}
                                        <a href="{{route('project.members',['id'=>$item->id])}}" class="project-user-btn" title="管理文档成员" data-toggle="tooltip" data-placement="bottom"  style="font-size: 12px;"><i class="fa fa-user-plus"></i> 用户</a>
                                        <a href="{{route('project.edit',['id'=>$item->id])}}" title="编辑项目" data-toggle="tooltip" data-placement="left"  style="font-size: 12px;"><i class="fa fa-pencil"></i> 编辑</a>
                                        {{--                                            @endif--}}
                                        {{--                                        @endif--}}
                                    </div>
                                ...
                                <div class="desc-text">
                                    @if(empty($item->description))
                                        &nbsp;
                                    @else
                                        <a href="{{route('document.index',['id'=>$item->id])}}" title="编辑文档" style="font-size: 12px;"  >
                                            {{$item->description}}
                                        </a>
                                    @endif
                                ...
                                <div class="info">
                                    <span title="创建时间" data-toggle="tooltip" data-placement="bottom"><i class="fa fa-clock-o"></i> {!! date('Y/m/d H:i',strtotime($item->created_at)) !!}</span>
                                    <span style="display: inline-block;padding-left: 10px;" title="创建者" data-toggle="tooltip" data-placement="bottom"><i class="fa fa-user"></i> {!! empty($item->nickname) ? '' : $item->nickname !!}</span>
