        ...
        <div class="manual-project-box">
            <div class="pull-left">
                <div class="manual-project-box-img">
                    <img src="{{asset('assets/doc/images/project_default.png')}}" width="50">
                </div>
                ...
                <div class="author">
                   <img src="{{$author_headimgurl}}" width="30" class="img-circle"> <span class="author-name">{{$author}}</span>
                    @if(empty($project->updated_at)=== false)
                    <span class="modify-time">最后更新时间 {!! date('Y年m月d日',strtotime($project->updated_at)) !!}</span>
                    @elseif(empty($project->created_at) === false)
                        <span class="modify-time">最后更新时间 {!! date('Y年m月d日',strtotime($project->created_at)) !!}</span>
                    @endif
                </div>
                <div class="manual-action">
                    ...
                    @else
                        <a href="{{route('doc.document.show',['id' => $first_document->id])}}" class="btn btn-success">阅读</a>
                    @endif
                </div>
            </div>
