...
namespace Modules\ApiTest\Entities;
...
class RouteInfo implements Arrayable, JsonSerializable
{
    ...
    public function __construct($route, $options = [])
    {
        ...
        $this->addMeta = config('apitest.route_meta');
        $this->analyzeRequests = config('apitest.analyze_requests');
    }
    ...
    {
        foreach(Module::all() as $_module) {
            $__alias = $_module->getAlias();
            $_moduleid = 'default';
            if(Str::startsWith($this->route->getName(), $__alias) || Str::contains($this->route->getName(), '.'.$__alias.'.')) {
                $_moduleid = $__alias;
                break;
            }
        }
