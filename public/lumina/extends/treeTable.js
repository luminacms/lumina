layui.define(['jquery'], function(exports) {
	...
	tree.prototype.template = function(e) {
		var t = this,
			...
			checkbox = e.is_checkbox ? '<div class="layui-unselect layui-form-checkbox cbx" lay-skin="primary"><i class="fa fa-check"></i></div>' : '',
		...
		o.each(t.data(e, e.data), function(idx, item) {
			var tr = '',
				is_checked = false,
				hide_class = (item[e.parent_field] == e.top_value) || (item[e.parent_field] == t.cache(e, item[e.parent_field])) ? '' : e.hide_class;
			// 设置每行数据层级
			item.level = level[item[e.primary_field]] = item[e.parent_field] != e.top_value ? (level[item[e.parent_field]] + 1) : 0;
			// 设置是否为最后一级
			item.is_end = !e.childs[item[e.primary_field]];
			o.each(e.cols, function(index, obj) {
				...
				// 标记设置行checkbox选中
				if(e.is_checkbox && e.checked && o.inArray(item[e.checked.field], e.checked.data) > -1) {
					is_checked = true;
				}
				// 第一次遍历头部的时候拼接表格头部
				idx || (thead += '<th ' + style + '>' + obj.title + '</th>');
				// 指定列加入开启、关闭小图标
				var icon = (obj.field == e.icon_field && !item.is_end) ? '<i class="mr-1 ' + (t.cache(e, item[e.primary_field]) ? e.icon.open : e.icon.close) + '"></i>' : '<span></span>';
				// 指定列小图标按照层级向后位移
				var left = (obj.field == e.icon_field ? level[item[e.primary_field]] * e.icon.left + 'px' : '');
				icon = icon.replace('>', ' style="margin-left:' + left + ';">');
				// 拼接行
				tr += '<td ' + style + (left ? 'data-down class="cursor-pointer"' : '') + '>' + icon + (is_table ? '' : (is_checked ? checked : checkbox)) + (obj.template ? obj.template(item) : item[obj.field]) + '</td>';
			});
			var box = is_table ? o(is_checked ? checked : checkbox).wrap('<td style="width:28px;">').parent().prop('outerHTML') : '';
			tbody += '<tr class="' + hide_class + '" data-id="' + item[e.primary_field] + '" data-pid="' + item[e.parent_field] + '" data-path="'+item['path']+'">' + box + tr + '</tr>';
		});
		// 处理表树和树的赋值模板
		...
		o(e.elem).html(table).off('click', click_btn).on('click', click_btn, function() {
            ...
            var  $self = $(this),
                is_checked = o(this).toggleClass('layui-form-checked').hasClass('layui-form-checked'),
				tr = o(this).parents('[data-id]'),
				id = tr.data('id'),
				pid = tr.data('pid');
			// t.childs_checkbox(e, id, is_checked);
            // t.parents_checkbox(e, pid);
            is_checked ? $self.addClass('layui-form-checked') : $self.removeClass('layui-form-checked');
			// 设置监听checkbox选择
	...
	// 点击收起循环隐藏子级元素
	tree.prototype.childs_hide = function(e, id, path) {
		var t = this;
        t.cache(e, id, false);

        o(e.elem).find('tbody>tr[data-path*=\''+path+'\']').each(function(){
            var $tr = $(this),
                i = $tr.find('i:not(.layui-icon-ok)'),
                _path = $tr.data('path');

            i.length && i.attr('class', e.icon.close);
            if(_path !== path && _path.indexOf(path) > -1) {
                // 子项
                $tr.addClass(e.hide_class)
            }
        })
    };
	// 重新组合数据，父子级关系跟随
	...
	tree.prototype.tree_data = function(e, lists, pid, data) {
		...
		o.each(e.data, function(index, item) {
			if(item[e.parent_field] == pid) {
				data.concat(t.tree_data(e, lists, item[e.primary_field], data))
			}
		});
		return data;
	};
	...
	tree.prototype.openAll = function(e) {
		var t = this;
		o.each(e.data, function(idx, item) {
			item[e.primary_field] && t.cache(e, item[e.primary_field], true);
		})
		t.render(e);
	}
	// 全部关闭
