layui.define(['laytpl'], function (e) {
    "use strict";
    var hint = layui.hint(),
        $ = layui.jquery,
        laytpl = layui.laytpl,
        ...
        job.prototype.pullData = function () {
            ...
            (!_config.cache && _config.ajax[_self.index] != undefined) && _config.ajax[_self.index].abort(), _config.ajax[_self.index] = $.ajax($.extend({
                ...
                url: _config.url,
                data: $.extend({
                    [_config.request.keywords]: _config.filter,
                    t: new Date().getTime()
                }, _config.params),
                ...
                if (e instanceof Object) {
                    layui.each(e, function (_i, _e) {
                        if (_e.toString().toLowerCase().indexOf(_config.filter.toLowerCase()) > -1) {
                            _config.temp_data[_self.index].push(e), _list.push(laytpl(_config.layout).render({
                                index: i,
        ...
        system.init = function (e, c) {
            ...
            _elems.each(function (_i, _e) {
                var _config = $.extend(system.config, {
                    elem: this,
                    url: $(_e).attr("lay-url")
                });
