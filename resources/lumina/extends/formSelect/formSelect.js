...
(function(layui, window, factory) {
    if(typeof exports === 'object') { // 支持 CommonJS
        ...
        layui.define(['jquery'], function(exports) {
            layui.link(layui.setter.base + "lib/extend/" + 'formSelect/formSelect.css');
            exports('formSelect', factory());
        });
    } else {
        window.formSelect = factory();
    }
    ...
    Select4.prototype.btns = function(id, btns, config){
        ...
        $.each(target, (key, val) => {
            ...
            if(btns.length){
                let show = config && config.show && (config.show == 'name' || config.show == 'Core.Resources.assets.layui.layer.icon') ? config.show : '';
